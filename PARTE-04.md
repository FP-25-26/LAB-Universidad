...
2. En el paquete `fp.universidad.tipos.test`, implemente una clase de test para probar los métodos del tipo `FactoriaUniversidad`, con las siguientes características:

   a. Defina métodos de test para probar cada uno de los métodos de la clase FactoriaUniversidad  que crean objetos bjetos a partir de cadenas de texto (`testCreaEspacio`, `testCreaDespacho`, `testCreaAsignatura`, `testCreaAlumno` y `testCreaNota`). Cada método de test debe recibir como entrada una cadena con los datos del objeto, invocar al método correspondiente de la factoría dentro de un bloque `try-catch` para capturar posibles excepciones y mostrar un mensaje de error en caso de que los datos sean incorrectos. Si la creación del objeto se ha realizado correctamente, el objeto se debe mostrar por la consola mostrando sus atributos más relevantes. Es aconsejable definir métodos auxiliares `mostrar` para mostrar estos atributos (`mostrarEspacio`, `mostrarDespacho`, `mostrarAsignatura`, `mostrarAlumno` y `mostrarNota`)
